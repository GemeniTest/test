...
class Stats:
    ...
    def increment_js_downloaded(self):
        with self.lock:
            self.js_downloaded_count += 1
...
def load_config():
    ...
    if 'host' in config and 'hosts' not in config:
        print("Detected old config format - converting 'host' to 'hosts' array...")
        config['hosts'] = [config['host']]
        with open(config_file, 'w') as f:
            json.dump(config, f, indent=2)
        print(f"Config updated successfully!")
    
    if 'hosts' in config and isinstance(config['hosts'], str):
...
def download_js_file(url, headers, js_folder):
    """Download a single JS file from URL and save it to JS folder"""
    try:
        filename = url.split('/')[-1].split('?')[0]
        if not filename.endswith('.js'):
            filename += '.js'
        
        save_path = os.path.join(js_folder, filename)
        
        response = download_file(url, headers)
        if response:
            with open(save_path, 'wb') as f:
                f.write(response.content)
            stats.increment_js_downloaded()
            safe_print(f"Downloaded JS: {filename}")
            return True
        else:
            safe_print(f"Failed to download: {url}")
            return False
...
def load_js_urls():
    """Load JS URLs from JS.txt file"""
    if not os.path.exists(JS_URLS_FILE):
        print(f"Error: '{JS_URLS_FILE}' not found. Creating template...")
        with open(JS_URLS_FILE, 'w') as f:
            f.write("# Add your JS URLs here, one per line\n")
            f.write("# Example:\n")
            f.write("# https://example.com/assets/main.js\n")
            f.write("# https://example.com/assets/vendor.js\n")
        print(f"Template created. Please add JS URLs to '{JS_URLS_FILE}'")
        exit()
    ...
    if not urls:
        print(f"Error: No URLs found in '{JS_URLS_FILE}'")
        exit()
...
def process_js_file(js_filename, executor, output_dir, map_cache_dir, headers, hosts):
    ...
    try:
        ...
        if os.path.exists(cached_map_path):
            ...
            map_urls = [urljoin(f"https://{host}/", map_filename) for host in hosts]
            
            def try_download_map(url):
                response = download_file(url, headers)
                if response:
                    try:
            ...
            with ThreadPoolExecutor(max_workers=len(hosts)) as map_executor:
                futures = [map_executor.submit(try_download_map, url) for url in map_urls]
        ...
        if has_embedded_content:
            ...
            def download_ts_parallel(source_path):
                for host in hosts:
                    ...
                    if response:
                        save_ts_content(response.text, save_path)
                        return True
                return False
            
            for source_path in ts_files:
                executor.submit(download_ts_parallel, source_path)
                
    except Exception as e:
        safe_print(f"Error processing {js_filename}: {e}")
...
def extract_from_ts_types(content, op_type):
    """
    Extracts operations from: export type Query = { ... }
    Handles both semicolon and non-semicolon delimited fields
    """
    operations = []
    
    # Match the type block - handles closing brace with or without semicolon
    op_block_pattern = re.compile(
        rf"export\s+type\s+{op_type}\s*=\s*\{{([\s\S]*?)\n\}}",
        re.MULTILINE | re.DOTALL
    )
    ...
    # Match fields without requiring semicolons: /** comment */ fieldName: ReturnType
    # Captures from comment start to end of line with return type
    ...
    for match in op_pattern.finditer(op_block_content):
        ...
        # Clean up the return type (trim trailing whitespace)
        ...
        operation_details["return_fields"] = extract_return_fields(return_type, content)
...
def extract_from_inline_queries(content):
    ...
    for pattern in inline_patterns:
        ...
        for match in matches:
            op_type_raw = match.group(1)
            op_name = match.group(2)
            args_str = match.group(3) if match.group(3) else ""
            
            op_type = op_type_raw.capitalize()
            
            operation_details = {
                "name": op_name,
                "description": f"Extracted from inline {op_type_raw} string.",
                "tags": [],
                "return_type": "See return fields",
                "return_fields": [],
                "arguments": [],
            ...
            if args_str:
                arg_matches = re.findall(r'\$([_A-Za-z]\w*)\s*:\s*([\[\]\w!]+)', args_str)
                for arg_name, arg_type_full in arg_matches:
                    is_required = arg_type_full.endswith('!')
                    arg_type_clean = re.sub(r'[!\[\]]', '', arg_type_full)
                    operation_details["arguments"].append({
...
def extract_from_inline_graphql(content):
    """
    Extracts mutations and queries from inline GraphQL format in JS files
    This captures mutations/queries that may be in template strings but already
    handled by extract_from_inline_queries - kept for safety/redundancy
    """
    operations = []
    return operations  # Return empty - extract_from_inline_queries handles these
...
def extract_graphql_operations(ts_dir):
    input_files = (
        ...
        glob.glob(os.path.join(ts_dir, '**/*.js'), recursive=True) +
        glob.glob(os.path.join(ts_dir, '**/*.jsx'), recursive=True) +
        glob.glob(os.path.join(ts_dir, '**/*.graphql'), recursive=True) +
        glob.glob(os.path.join(ts_dir, '**/*.gql'), recursive=True)
    )
    
    # Also process JS files from local JS folder
    if os.path.exists(JS_FOLDER):
        input_files += (
            glob.glob(os.path.join(JS_FOLDER, '**/*.js'), recursive=True) +
            glob.glob(os.path.join(JS_FOLDER, '**/*.jsx'), recursive=True)
        )

    if not input_files:
        print("No TS/JS/GraphQL files found for extraction")
        return {}
    ...
    for filename in input_files:
        try:
            ...
            if filename.endswith(('.graphql', '.gql')):
                ...
                for op_type, op in extract_from_inline_queries(content):
                    all_operations[(op_type, op['name'])] = op
                
                for op_type, op in extract_from_inline_graphql(content):
                    all_operations[(op_type, op['name'])] = op
...
def compare_and_save_operations(current_operations):
    ...
    # Step 1: Save CURRENT history first (before clearing NEW folder)
    print(f"\nStep 1: Updating history with current operations...")
    save_history(current_operations)

    # Step 2: Clear old GraphQL_NEW directory
    if os.path.exists(GRAPHQL_NEW_DIR):
        print(f"Step 2: Clearing old GraphQL_NEW directory...")
        import shutil
        shutil.rmtree(GRAPHQL_NEW_DIR)
        print("  ✓ GraphQL_NEW cleared")

    # Step 3: Write all current operations to main API
    print(f"\nStep 3: Writing all operations to {GRAPHQL_API_DIR}...")
    ...
    # Step 4: Write ONLY new operations (from this run) to GraphQL_NEW
    if new_operations:
        print(f"\n{'='*60}")
        print(f"🎯 FOUND {len(new_operations)} NEW OPERATIONS THIS RUN!")
        print(f"{'='*60}")
        print(f"Writing NEW operations to {GRAPHQL_NEW_DIR}...")
        ...
        print(f"  🔴 NEW Mutations: {len(new_mutations)}")
        for op_name in sorted(new_mutations.keys()):
            print(f"     ✓ {op_name}")
        
        if new_queries:
            print(f"  🔵 NEW Queries: {len(new_queries)}")
            for op_name in sorted(new_queries.keys()):
                print(f"     ✓ {op_name}")
        
        if new_subscriptions:
            print(f"  🟢 NEW Subscriptions: {len(new_subscriptions)}")
            for op_name in sorted(new_subscriptions.keys()):
                print(f"     ✓ {op_name}")
    else:
        print(f"\n{'='*60}")
        print("✓ No new operations found this run")
        print(f"{'='*60}")
    ...
    new_mutations_count = len({k[1]: v for k, v in new_operations.items() if k[0] == 'Mutation'})
    new_queries_count = len({k[1]: v for k, v in new_operations.items() if k[0] == 'Query'})
    new_subscriptions_count = len({k[1]: v for k, v in new_operations.items() if k[0] == 'Subscription'})
    
    print(f"  New Mutations: {new_mutations_count}")
    print(f"  New Queries: {new_queries_count}")
    print(f"  New Subscriptions: {new_subscriptions_count}")
    print(f"  TOTAL NEW: {len(new_operations)}")

def main():
    print("=" * 60)
    print("GraphQL Monitor v3.0 - JS URL Edition")
    print("=" * 60)
    print("Features:")
    print("  ✓ Download JS files from URLs in JS.txt")
    print("  ✓ Original TypeScript extraction")
    print("  ✓ Inline queries extraction")
    print("  ✓ gql template literals")
    print("  ✓ .graphql/.gql schema files")
    print("  ✓ Ultra-fast downloads (50 workers)")
    print("  ✓ Old config format support")
    print("=" * 60)
    print()
    ...
    # Step 0: Download JS files from URLs
    print(f"Step 0: Downloading JS files from {JS_URLS_FILE}")
    print("=" * 60)
    
    js_urls = load_js_urls()
    stats.js_total_count = len(js_urls)
    
    print(f"Found {stats.js_total_count} URLs to download")
    print(f"Using {MAX_WORKERS} parallel workers...\n")
    
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [executor.submit(download_js_file, url, headers, JS_FOLDER) for url in js_urls]
        for future in futures:
            try:
                future.result()
            except Exception as e:
    ...
    print(f"\nJS Download Summary:")
    print(f"URLs processed: {stats.js_total_count}")
    print(f"JS files downloaded: {stats.js_downloaded_count}")
    
    print(f"\nStep 1: Downloading TypeScript files from source maps")
    print(f"Hosts: {', '.join(hosts)}\n")

    if not os.path.exists(JS_FOLDER):
        print(f"Error: '{JS_FOLDER}' folder not found.")
        return
    
    local_js_files = [f for f in glob.glob(os.path.join(JS_FOLDER, '*.js*')) if not f.endswith('.map')]
    
    if not local_js_files:
        print("No JS files found. Skipping TypeScript extraction.")
    else:
        print(f"Found {len(local_js_files)} JS files for processing")
        print(f"Using {MAX_WORKERS} parallel workers with aggressive connection pooling...\n")
        
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = [executor.submit(process_js_file, js_file, executor, TS_DIR, MAP_CACHE_DIR, headers, hosts) 
        ...
        print(f"\nDownload Summary:")
        print(f"JS files processed: {len(local_js_files)}")
    ...
    print("Using ALL extraction patterns (original + new)...")
    ...
    print(f"Extracted {len(all_operations)} total operations")
